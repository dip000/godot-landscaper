...
func is_chunkified(element:SceneElement) -> bool:
	var anchor_node:Node = element.anchor_node
	if not anchor_node: return false
	
	var original_mmi:MultiMeshInstance3D = element.anchor_node.get_node_or_null(NodePath(element.name))
	if not original_mmi: return false
	return element.anchor_node.has_node(chunks_parent_name)
...
func run(element:SceneElement, save_data:GLSaveData) -> String:
	...
	var result = await reset(element, save_data)
	if result != "OK": return "Reset failed. %s" %result
	await _frame()
	
	original_mmi.hide()
	
	var root_parent:Node = SceneManager.find_or_create_node(Node3D, element.anchor_node, chunks_parent_name)
...
func reset(element:SceneElement, save_data:GLSaveData) -> String:
	...
	for chunk in root_parent.get_children():
		for variant in chunk.get_children():
			if variant.name.begins_with(element.name):
				variant.queue_free()
	
	# Cleanup chunk holders
	await _frame()
	for chunk in root_parent.get_children():
	...
	# Cleanup chunk root
	await _frame()
	if root_parent.get_child_count() <= 0:

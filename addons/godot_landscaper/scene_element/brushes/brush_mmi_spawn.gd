@tool
extends Brush
class_name ActionMMISpawn

# Should be injected before calling start()
var mmi:MultiMeshInstance3D
var mm:MultiMesh
var grass:Grass3D
var save_data:GLSaveData


func start(hit_info:Dictionary):
	mm = mmi.multimesh
	if save_data.size_base.is_zero_approx():
		GLDebug.warning("Grass volume is zero. Used Vector3.ONE")
		save_data.size_base = Vector3.ONE
	if save_data.transforms.size() > mm.instance_count:
		GLDebug.warning("There were more stored data values than the current multimesh values. Multimesh values will be replaced")
		rebuild()
	elif save_data.transforms.size() == 0 and mm.instance_count > 0:
		GLDebug.warning("New project data values were recreated from the current multimesh values.")
		dump()
	Scanner.clear_cache()

func primary(hit_info:Dictionary):
	_add_radial( hit_info )
	rebuild()

func secondary(hit_info:Dictionary):
	_get_remove_radial( hit_info )
	rebuild()

func end():
	Scanner.clear_cache()


# Gets every MultiMesh transform except the ones inside the brush
func _get_remove_radial(hit_info:Dictionary):
	var brush_radius_sqr:float = pow( Landscaper.scene.brush.get_scale_ratio()*0.5, 2)
	var mouse_world_pos:Vector3 = hit_info.position
	save_data.transforms.clear()
	save_data.bottom_colors.clear()
	save_data.top_colors.clear()
	
	for i in mm.instance_count:
		var instance_transform:Transform3D = mm.get_instance_transform(i)
		var instance_world_pos:Vector3 = mmi.to_global( instance_transform.origin )
		var dist_sqr:float = instance_world_pos.distance_squared_to( mouse_world_pos )
		
		if dist_sqr > brush_radius_sqr or grass.erase_ratio < randf():
			save_data.transforms.append( instance_transform )
			save_data.bottom_colors.append(  mm.get_instance_color(i) )
			save_data.top_colors.append(  mm.get_instance_custom_data(i) )


func _add_radial(hit_info:Dictionary):
	var brush_radius:float = Landscaper.scene.brush.get_scale_ratio()*0.5
	var mouse_world_pos:Vector3 = hit_info.position
	var raycaster:SceneRaycaster = Landscaper.scene.raycaster
	
	for i in range(grass.spawn_ratio):
		# Two random points over the brush sphere to make a ray
		var sphere_global_point1:Vector3 = _get_surface_point(brush_radius) + mouse_world_pos
		var sphere_global_point2:Vector3 = _get_surface_point(brush_radius) + mouse_world_pos
		
		var result:Dictionary = raycaster.point_to_point(sphere_global_point1, sphere_global_point2)
		if not result:
			continue
		
		# Await a frame if the cache was newly created so it can be re-scanned
		var cache:Scanner.Cache = Scanner.cache_scan( result.collider, grass, true )
		if cache.new:
			await Engine.get_main_loop().process_frame
			result = raycaster.point_to_point(sphere_global_point1, sphere_global_point2)
			if not result:
				continue
		
		# Align Normals. Add a little offset so it doesn't throw errors on axis alignment
		# This will actually make the grass "look" up at the sky instead of standing at 90°
		var basis := Basis.looking_at(result.normal + Vector3.ONE*0.01)
		
		# Compose rotation using quaternion magic
		basis *= Basis(
			# PI*0.5 on X axis compenzates for looking at the sky as mentioned before
			Quaternion(Vector3.RIGHT, save_data.rotation_base.x - PI * 0.5) *
			Quaternion(Vector3.UP, save_data.rotation_base.y) *
			Quaternion(Vector3.FORWARD, save_data.rotation_base.z) *
			Quaternion(Vector3.RIGHT, randf()*save_data.rotation_randomize.x) *
			Quaternion(Vector3.UP, randf()*save_data.rotation_randomize.y) *
			Quaternion(Vector3.FORWARD, randf()*save_data.rotation_randomize.z)
		)
		
		# to_local() takes rotation and scale in consideration
		var local_pos:Vector3 = mmi.to_local( result.position )
		
		# Save base and random values
		var local_transf := Transform3D( basis, local_pos )
		var size_offset:Vector3 = save_data.size_randomize * _randv(0, 1)
		
		local_transf = local_transf.scaled_local( save_data.size_base + size_offset )
		save_data.transforms.append( local_transf )
		
		# Save colors. Use cached colors for performance
		var color:Color = Scanner.scan_color( result, cache )
		save_data.bottom_colors.append( color )
		save_data.top_colors.append( grass.primary_color )


func rebuild():
	mm.instance_count = save_data.transforms.size()
	for i in range(mm.instance_count):
		mm.set_instance_transform( i, save_data.transforms[i] )
		mm.set_instance_color( i, save_data.bottom_colors[i] )
		mm.set_instance_custom_data( i, save_data.top_colors[i] )


func dump():
	save_data.transforms.resize(mm.instance_count)
	save_data.bottom_colors.resize(mm.instance_count)
	save_data.top_colors.resize(mm.instance_count)
	for i in mm.instance_count:
		save_data.transforms[i] = mm.get_instance_transform( i )
		save_data.bottom_colors[i] = mm.get_instance_color( i )
		save_data.top_colors[i] = mm.get_instance_custom_data( i )


func _get_surface_point(radius:float) -> Vector3:
	var point:Vector3 = _randv(-1, +1).normalized()
	return point * radius

func _randv(min:float, max:float) -> Vector3:
	return Vector3( randf_range(min,max), randf_range(min,max), randf_range(min,max) )

...
static var _ray_surfaces := PhysicsRayQueryParameters3D.new()
static var _direct_space_state:PhysicsDirectSpaceState3D
...
static func update_hit_info(cam:Camera3D, mouse_pos:Vector2) -> Dictionary:
...
static func set_collision_mask(collision_mask:int):
	_ray_surfaces.collision_mask = collision_mask


static func point_to_point(from:Vector3, to:Vector3) -> Dictionary:
	if _direct_space_state:

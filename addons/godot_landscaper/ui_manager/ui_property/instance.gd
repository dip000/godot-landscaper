...
@onready var _randomness:CustomRange = $Content/MarginBody/VBoxContainer/CustomSlider
...
var randomness:float:
	get: return _randomness.value
	set(v): _randomness.value = v
...
func _ready():
	...
	_randomness.on_change.connect( _on_randomness_changed )

func _on_toggled(button_pressed:bool):
	_set_enable( button_pressed )

func _on_file_changed():
	if not FileAccess.file_exists( _file.path ):
		scene = null
		return
	
	var resource:Resource = load( _file.path )

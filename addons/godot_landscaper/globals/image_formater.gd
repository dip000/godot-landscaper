...
static func soft_clean_image(image:Image, format:Image.Format, size:Vector2i=Vector2i.ZERO):
	if not image:
		return
...
static func hard_clean_image(image:Image, format:Image.Format, size:Vector2i, color:Color=Color.TRANSPARENT) -> Image:
	if not image:

...
static func error(msg:Variant):
	print_rich("[color=#d66960][b]ERROR [Landscaper][/b] %s[/color]" %msg)

static func warning(msg:Variant):
	if debugging_warnings():
		print_rich("[color=#ffde66][b]WARN  [Landscaper][/b] %s[/color]" %msg)

static func state(msg:Variant):
	if debugging_states():
		print_rich("[color=#db7093][b]STATE [Landscaper] [/b]%s[/color]" %msg)

static func internal(msg:Variant):
	if debugging_internal():
		print_rich("[color=#bbbbbb][b]INTER [Landscaper] [/b]%s[/color]" %msg)

static func spam(msg:Variant):
	if debugging_spam():
		print_rich("[color=#e0e0e088][b]SPAM  [Landscaper] [/b]%s[/color]" %msg)
